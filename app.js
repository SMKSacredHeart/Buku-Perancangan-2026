...
function renderCalendar() {
  ...
  container.innerHTML = sortedEvents.map(evt => {
    ...
    return `
      <div class="glass-panel calendar-event-row category-${evt.category.toLowerCase()}">
...
function setupEventListeners() {
  ...
  navLinks.forEach(link => {
    link.addEventListener("click", (e) => {
      ...
      if (headerSubtitle) {
        ...
        if (tabName === "carta") headerSubtitle.innerText = "Carta organisasi pengurusan dan penyelaras sekolah.";
      }
  ...
  // Org Chart Filter Buttons
  const cartaFilterBtns = document.querySelectorAll(".carta-filter-btn");
  cartaFilterBtns.forEach(btn => {
    btn.addEventListener("click", () => {
      cartaFilterBtns.forEach(b => b.classList.remove("active"));
      btn.classList.add("active");
      state.cartaFilter = btn.getAttribute("data-category");
      renderOrgChart();
    });
  });

  // Org Chart Search Input
  const cartaSearchEl = document.getElementById("carta-search");
  if (cartaSearchEl) {
    cartaSearchEl.addEventListener("input", (e) => {
      state.cartaSearch = e.target.value;
      renderOrgChart();
    });
  }
}
...
// Render School Organizational Chart
function renderOrgChart() {
  const container = document.getElementById("org-chart-container");
  if (!container) return;

  const org = schoolData.organization;
  if (!org) return;

  // Helper function to build card HTML
  const createCardHTML = (person, isCoordinator = false) => {
    const hasImage = !!person.image;
    const avatarHTML = hasImage 
      ? `<img src="${person.image}" alt="${person.name}" class="org-avatar" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
         <div class="org-avatar-placeholder" style="display:none;">${person.name.charAt(0)}</div>`
      : `<div class="org-avatar-placeholder">${person.name.charAt(0)}</div>`;

    // Match status
    const matchesSearch = !state.cartaSearch || 
      person.name.toLowerCase().includes(state.cartaSearch.toLowerCase()) || 
      person.role.toLowerCase().includes(state.cartaSearch.toLowerCase());
    
    const matchesFilter = state.cartaFilter === 'all' || 
      person.category.toLowerCase() === state.cartaFilter.toLowerCase();

    let cardClass = isCoordinator ? 'org-card org-card-coordinator' : 'org-card';
    if (state.cartaSearch || state.cartaFilter !== 'all') {
    ...
    return `
      <div class="${cardClass}" data-name="${person.name}" data-role="${person.role}" data-category="${person.category}">
        <div class="org-avatar-container">
          ${avatarHTML}
        </div>
        <div class="org-card-info">
          <span class="org-role-badge ${person.category.toLowerCase()}">${person.role}</span>
  ...
  // Build Tier 1: Pengetua
  const tier1HTML = `
    <div class="org-level org-level-1">
      ${createCardHTML(org.principal)}
    </div>
  `;
  ...
  const tier2HTML = `
    <div class="org-level org-level-2">
      ${org.pkList.map(pk => createCardHTML(pk)).join("")}
    </div>
  `;
  ...
  const tier3HTML = `
    <div class="org-level org-level-3">
      ${org.gkmpList.map(gk => createCardHTML(gk)).join("")}
    </div>
  `;

  // Build Tier 4: Coordinators Section (with header title and two rows)
  const tier4HTML = `
    <div class="org-coordinators-section">
      <div class="org-coordinators-title">Penyelaras Pengurusan & Pentadbiran Sekolah</div>
      <div class="org-grids-container">
        <div class="org-grid-8">
          ${org.coordinatorsRow1.map(coord => createCardHTML(coord, true)).join("")}
        </div>
        <div class="org-grid-13">
          ${org.coordinatorsRow2.map(coord => createCardHTML(coord, true)).join("")}
        </div>
      </div>
    </div>
  `;
